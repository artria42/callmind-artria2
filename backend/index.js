 ...
 * 🔥 ИСПРАВЛЕННАЯ ВЕРСИЯ — строгие инструкции, медицинский словарь
 */
...
КОНТЕКСТ: Звонок записан в стерео — каждый канал это отдельный человек. Текст идёт блоками, потому что каждый канал распознан отдельно.

ЗАДАЧА 1 — ПЕРЕВОД:
- Если текст на казахском → переведи на русский ДОСЛОВНО, сохраняя все детали
- Если на русском → оставь как есть, исправь только очевидные ошибки распознавания
- Используй медицинский словарь:
  буын = сустав
  омыртқа = позвоночник
  бел = поясница
  тізе = колено
  ...
  жазылу = запись
  тегін = бесплатно
  9900 теңге = 9900 тенге

ЗАДАЧА 2 — СТРУКТУРА ДИАЛОГА:
- Раздели сплошной блок каждого спикера на логичные реплики (1-3 предложения)
- Чередуй АДМИНИСТРАТОРА и ПАЦИЕНТА в хронологическом порядке
- Роли НЕ МЕНЯЙ — что было manager остаётся manager

СТРОГИЕ ЗАПРЕТЫ:
❌ НЕ выдумывай слова, которых нет
❌ НЕ пропускай предложения
❌ НЕ меняй смысл
❌ НЕ добавляй вежливые фразы
❌ НЕ сокращай информацию

ПРИМЕР:
Вход:
[АДМИНИСТРАТОР]
Сәлеметсіз бе добрый день клиника Мирамед буындар мен омыртқаны емдейміз

[ПАЦИЕНТ]
Менің белім қатты ауырады екі апта болды

Выход:
[
  {"role": "manager", "text": "Здравствуйте, добрый день, клиника Мирамед. Мы лечим суставы и позвоночник."},
  {"role": "client", "text": "У меня поясница сильно болит, уже две недели."}
]

ФОРМАТ — JSON массив. Без markdown, без комментариев.
...
/**
 * Fallback: GPT-4o определяет роли + переводит (моно режим)
 */
...
КОНТЕКСТ: Моно-запись звонка (оба голоса в одном канале). Разговор на казахском, русском или смеси.

ЗАДАЧИ:
1. Переведи всё на грамотный русский (используй медицинский словарь):
   буын=сустав, омыртқа=позвоночник, бел=поясница, тізе=колено, иық=плечо
   ауырады=болит, емхана=клиника, дәрігер=врач, тексеру=обследование
   
2. Исправь ошибки Whisper

3. Определи роли:
   - АДМИНИСТРАТОР (manager): приветствует, называет клинику, предлагает запись
   - ПАЦИЕНТ (client): описывает боли, спрашивает цены
