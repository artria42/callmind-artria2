 ...
 * РАДИКАЛЬНЫЙ ПОДХОД - пронумерованные предложения для полного контроля
 */
async function syncAndTranslateChannels(adminText, clientText) {
  // Разбиваем на предложения и нумеруем
  const adminSentences = adminText.match(/[^.!?]+[.!?]+/g) || [adminText];
  const clientSentences = clientText.match(/[^.!?]+[.!?]+/g) || [clientText];
  
  const adminNumbered = adminSentences.map((s, i) => `A${i+1}. ${s.trim()}`).join('\n');
  const clientNumbered = clientSentences.map((s, i) => `П${i+1}. ${s.trim()}`).join('\n');

  const prompt = `# ЗАДАЧА
Перед тобой ПРОНУМЕРОВАННЫЕ предложения из двух каналов телефонного разговора клиники.
Канал А = Администратор (role: "manager")
Канал П = Пациент (role: "client")

ТЫ ДОЛЖЕН:
1. ПЕРЕВЕСТИ каждое предложение с казахского/смешанного на русский
2. СИНХРОНИЗИРОВАТЬ их в логичный диалог (вопрос-ответ)
3. Вернуть JSON массив

# КРИТИЧЕСКИ ВАЖНО ⚠️
- КАЖДОЕ предложение из списка ниже ДОЛЖНО быть в результате
- НЕ пропускай НИЧЕГО (ни А1, ни А2... ни П1, ни П2...)
- НЕ добавляй новых предложений которых нет в списке
- Просто ПЕРЕВЕДИ и расставь в правильном порядке

# МЕДИЦИНСКИЕ ТЕРМИНЫ
буын->сустав, омыртқа->позвоночник, бел->поясница, тізе->колено
ауырады->болит, дәрігер->врач, емхана->клиника
зейнетақы->пенсия, келесі апта->следующая неделя, ертең->завтра
жоқ->нет, иә/ия->да, қайта қоңырау шалу->перезвонить

# ФОРМАТ ОТВЕТА
JSON массив с полями role и text:
[
  {"role": "manager", "text": "перевод A1"},
  {"role": "client", "text": "перевод П1"},
  {"role": "manager", "text": "перевод A2"}
]

БЕЗ markdown, БЕЗ комментариев, ТОЛЬКО JSON.

# КАНАЛ А (АДМИНИСТРАТОР)
${adminNumbered}

# КАНАЛ П (ПАЦИЕНТ)
${clientNumbered}`;

  console.log('🧠 GPT-4o: пронумерованный перевод...');
  console.log(`📝 Предложений: А=${adminSentences.length}, П=${clientSentences.length}`);
  ...
  console.log(`✅ Результат: ${formatted.length} реплик (ожидалось ~${adminSentences.length + clientSentences.length})`);
  
  // ПРОВЕРКА: если получилось меньше 80% от ожидаемого - предупреждение
  const expected = adminSentences.length + clientSentences.length;
  if (formatted.length < expected * 0.8) {
    console.warn(`⚠️ ПОТЕРЯ ДАННЫХ: получено ${formatted.length} из ${expected} предложений`);
  }
