...
async function translateChannel(text, channelName) {
  ...
  const prompt = `Переведи этот текст с казахского/смешанного языка на ГРАМОТНЫЙ русский.

ПРАВИЛА:
- Переводи ДОСЛОВНО, сохраняя все детали
- Медицинские термины: буын=сустав, омыртқа=позвоночник, бел=поясница, тізе=колено, ауырады=болит, дәрігер=врач, емхана=клиника, зейнетақы=пенсия
- Исправляй ошибки распознавания речи
- НЕ добавляй отсебятины
- НЕ делай пересказ - переводи каждое предложение

ТЕКСТ:
${text}

Верни ТОЛЬКО перевод на русском, без комментариев.`;
