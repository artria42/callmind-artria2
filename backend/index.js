 ...
 * ПРОСТОЙ подход с логированием исходного текста
 */
async function syncAndTranslateChannels(adminText, clientText) {
  ...
  console.log('\n[АДМИН КАНАЛ]');
  console.log(adminText.substring(0, 500) + (adminText.length > 500 ? '...' : ''));
  console.log('\n[ПАЦИЕНТ КАНАЛ]');
  console.log(clientText.substring(0, 500) + (clientText.length > 500 ? '...' : ''));
  console.log('='.repeat(60) + '\n');

  const prompt = `Ты — переводчик телефонных разговоров клиники.

У тебя есть ДВА БЛОКА ТЕКСТА:
1. Канал администратора (role: "manager")
2. Канал пациента (role: "client")

ЗАДАЧА:
1. Перевести с казахского на русский
2. Разбить на короткие реплики (1-2 предложения)
3. Расставить в логичном порядке диалога (вопрос-ответ)

СЛОВАРЬ:
буын->сустав, омыртқа->позвоночник, бел->поясница, тізе->колено, ауырады->болит
дәрігер->врач, емхана->клиника, зейнетақы->пенсия, келесі апта->следующая неделя
ертең->завтра, жоқ->нет, иә->да, қайта қоңырау->перезвонить

ПРАВИЛА:
- Переводи ВСЁ что есть в исходнике
- НЕ выдумывай новые фразы
- НЕ пропускай упоминания денег, пенсии, отказов
- Сохраняй смысл: если отказ - пиши отказ, если согласие - пиши согласие

ФОРМАТ:
[{"role": "manager", "text": "..."}, {"role": "client", "text": "..."}]

КАНАЛ АДМИНИСТРАТОРА:
${adminText}

КАНАЛ ПАЦИЕНТА:
${clientText}`;

  console.log('🧠 GPT-4o синхронизация...');
