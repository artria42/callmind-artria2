...
async function translateToRussian(text) {
  ...
  console.log('🌐 Translating Kazakh → Russian...');
  try {
    const response = await axios.post(GOOGLE_PROXY_URL, {
      type: 'chat', apiKey: OPENAI_API_KEY, model: 'gpt-4o-mini', max_tokens: 4000,
      messages: [{ role: 'user', content: `Переведи с казахского на русский. Если текст уже на русском — верни без изменений. Верни ТОЛЬКО перевод:\n\n${text}` }]
    }, { timeout: 60000 });
