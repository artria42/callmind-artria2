...
/**
 * GPT-4o: только перевод (роли уже определены по каналам)
 */
async function translateDialogue(formatted) {
  ...
  const prompt = `Ты — переводчик для медицинской клиники MIRAMED (Актобе, Казахстан).
Клиника лечит суставы и позвоночник без операции.

Тебе дан диалог телефонного звонка. Роли (АДМИНИСТРАТОР/ПАЦИЕНТ) уже определены правильно по аудиоканалам — НЕ МЕНЯЙ их.
Разговор может быть на казахском, русском или на смеси обоих языков.

ЗАДАЧИ:
1. Переведи весь текст на грамотный русский язык
2. Исправь ошибки распознавания речи (Whisper)
3. Медицинские термины: буын=сустав, омыртқа=позвоночник, ауырады=болит, бел=поясница, тізе=колено, иық=плечо, бас=голова, мойын=шея
4. Убери мусорные звуки (ммм, ааа, эээ) если не несут смысла
5. НЕ добавляй слов, которых не было в оригинале
6. НЕ МЕНЯЙ роли — они точные

ФОРМАТ — строго JSON массив:
[
  {"role": "manager", "text": "Переведённый текст"},
  {"role": "client", "text": "Переведённый текст"}
]
...
${dialogText}`;
  ...
  // Защита: если GPT вернул другое кол-во реплик, сохраняем роли из оригинала
  translated = translated
    .filter(item => item.text && item.text.trim().length > 0)
    .map((item, i) => ({
      role: formatted[i]?.role || (item.role === 'client' ? 'client' : 'manager'),
      text: item.text.trim()
    }));
