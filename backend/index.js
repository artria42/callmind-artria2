 ...
 * УСИЛЕННАЯ ВЕРСИЯ - обрабатывает ВЕСЬ текст от начала до конца
 */
...
ОБРАБОТАЙ КАЖДОЕ ПРЕДЛОЖЕНИЕ ИЗ ОБОИХ КАНАЛОВ.
НИ ОДНА ФРАЗА НЕ ДОЛЖНА БЫТЬ ПРОПУЩЕНА.
Начинай с ПЕРВОГО слова из каналов и заканчивай ПОСЛЕДНИМ словом.
...
- Медицинские термины переводи строго:
  ...
  ауырады -> болит, дәрігер -> врач, емхана -> клиника, тексеру -> обследование
  жазылу -> записаться на приём, зейнетақы -> пенсия, келесі апта -> следующая неделя

## ШАГ 2: СИНХРОНИЗАЦИЯ (САМОЕ ВАЖНОЕ)

⚠️ ОБЯЗАТЕЛЬНО ВКЛЮЧАЙ В РЕЗУЛЬТАТ:

**НАЧАЛО РАЗГОВОРА (первые реплики):**
- "Алло" / "Сәлем" / "Саламатсыз ба" 
- "Здравствуйте, клиника Мирамед"
- Любые приветствия

**СЕРЕДИНА (вся основная часть):**
- Все вопросы о боли
- Все ответы пациента
- Предложения услуг
- Обсуждение цен
- Вопросы о враче
- Любые упоминания Instagram / рекламы

**КОНЕЦ РАЗГОВОРА (последние реплики):**
- Договорённости о времени ("завтра", "на следующей неделе")
- Упоминания денег ("пенсия", "когда деньги получу", "когда зарплата")
- Прощания ("Спасибо", "До свидания", "Сау болыңыз", "Перезвоню")
- "Вы на этом номере?" / "Я перезвоню"
...
- Чередуй роли естественным образом.
...
# RESTRICTIONS
❌ НЕ ПРОПУСКАЙ НИ ОДНОЙ РЕПЛИКИ из исходного текста
❌ НЕ добавляй отсебятины
❌ НЕ сокращай длинные фразы - переведи их полностью
❌ НЕ пиши markdown (\`\`\`json)

# КАЧЕСТВО-КОНТРОЛЬ
После формирования JSON проверь:
✅ Есть ли приветствие в начале? (Если нет - ДОБАВЬ из исходника)
✅ Есть ли прощание в конце? (Если нет - ДОБАВЬ из исходника)
✅ Все ли упоминания денег/времени включены?
✅ Весь ли текст из обоих каналов использован?
...
${clientText}`;
  ...
  console.log('🧠 GPT-4o: полная синхронизация (от начала до конца)...');
  const response = await axios.post(GOOGLE_PROXY_URL, {
    type: 'chat', 
    apiKey: OPENAI_API_KEY, 
    model: 'gpt-4o', 
    max_tokens: 5000,  // Увеличил лимит
    temperature: 0.1,
  ...
  console.log(`📊 Результат: ${formatted.length} реплик из ~${(adminText.length + clientText.length) / 50} ожидаемых`);
