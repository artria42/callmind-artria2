...
//
//  ФОРМАТ ВЫВОДА v5.1: Массив реплик диалога по очереди
//  GPT-4o восстанавливает порядок реплик по контексту двух каналов
// ====================================================================
...
/**
 * GPT-4o: Перевод двух каналов → реконструкция диалога репликами
 *
 * НОВАЯ ЛОГИКА v5.1:
 * Получаем два канала (админ + клиент), GPT-4o восстанавливает порядок реплик
 * по контексту диалога и переводит каждую реплику на русский.
 *
 * Формат: { dialogue: [{ role: "manager", text: "..." }, { role: "client", text: "..." }, ...] }
 */
...
Ты — эксперт по реконструкции медицинских телефонных разговоров.

# ЗАДАЧА
У тебя есть ДВА КАНАЛА аудио из звонка в клинику "Мирамед" (Актобе):
1. КАНАЛ АДМИНИСТРАТОРА — всё что сказал менеджер клиники (может быть на казахском/русском/миксе)
2. КАНАЛ ПАЦИЕНТА — всё что сказал пациент (может быть на казахском/русском/миксе)

Каналы записаны раздельно, но НЕ содержат временных меток.

Твоя задача:
1. **ПОНЯТЬ КОНТЕКСТ**: Прочитай оба канала, пойми о чём разговор (звонок про лечение суставов)
2. **ВОССТАНОВИТЬ ПОРЯДОК**: По логике диалога определи, кто что говорил и в каком порядке
   - Администратор обычно начинает: "Алло, [Имя]?", "Здравствуйте", "Клиника Мирамед"
   - Пациент отвечает на вопросы, описывает симптомы, задаёт вопросы о цене
   - Используй логику вопрос→ответ, приветствие→приветствие
3. **ПЕРЕВЕСТИ НА РУССКИЙ**: Каждую реплику переведи на чистый русский (если была на казахском)
4. **РАЗБИТЬ НА РЕПЛИКИ**: Верни диалог как последовательность реплик (кто говорил → что сказал)
...
# ПРАВИЛА
1. Переводи ДОСЛОВНО — не выдумывай, не добавляй от себя
2. Если непонятно что сказано — переведи как понял, но НЕ придумывай
3. Убирай повторы и мусорные фрагменты распознавания
4. Каждая реплика = одна логическая фраза или группа фраз от одного говорящего
5. Если администратор долго говорил без перерыва — можно разбить на 2-3 реплики по смыслу
6. НЕ объединяй вопрос и ответ в одну реплику — это РАЗНЫЕ реплики

# КОНТЕКСТ КЛИНИКИ МИРАМЕД
- Лечение суставов и позвоночника без операции
- Оффер: "Экспертная диагностика" = Консультация + УЗИ двух суставов + бесплатный повторный приём = 9 900 тенге
- Типичный звонок: приветствие → выявление боли → презентация оффера → запись на приём

# ФОРМАТ ОТВЕТА — строго JSON, без markdown:
{
  "dialogue": [
    { "role": "manager", "text": "Алло, здравствуйте!" },
    { "role": "client", "text": "Здравствуйте." },
    { "role": "manager", "text": "Меня зовут Айгуль, клиника Мирамед. Вы оставляли заявку по поводу лечения суставов?" },
    { "role": "client", "text": "Да, у меня колено болит." }
  ]
}

ВАЖНО: role может быть только "manager" или "client". Никакие другие значения не допускаются.`;

  const userPrompt = `КАНАЛ АДМИНИСТРАТОРА (сырой текст):
${adminRawText}

КАНАЛ ПАЦИЕНТА (сырой текст):
${clientRawText}

Восстанови диалог репликами, переведи на русский, верни JSON.`;

  logger.info('🧠 GPT-4o: реконструкция диалога + перевод...');
  ...
  try {
    ...
    if (match) {
      ...
      logger.warn('⚠️ GPT не вернул JSON, fallback к двум блокам', {
        contentPreview: content.substring(0, 200)
      });
      // Fallback: если GPT не смог восстановить диалог, возвращаем два блока
      return {
        dialogue: [
          { role: 'manager', text: adminRawText },
          { role: 'client', text: clientRawText }
        ]
      };
    }
  }
  ...
  logger.info(`✅ Диалог восстановлен`, {
    replicas: dialogue.length,
    firstReplica: dialogue[0]?.text?.substring(0, 50)
  });
...
/**
 * GPT-4o: Перевод моно-аудио (оба голоса в одном канале) → реконструкция диалога репликами
 *
 * НОВАЯ ЛОГИКА v5.1:
 * Из моно-транскрипта GPT-4o восстанавливает диалог репликами,
 * определяя роли по контексту и переводя на русский.
 */
async function repairAndTranslateMono(rawText) {
  if (!rawText || rawText.trim().length < 15) {
    return { dialogue: [{ role: 'manager', text: rawText || '' }] };
  }
...
Транскрипт может быть на казахском, русском или миксе языков.

Твоя задача:
1. **РАЗДЕЛИТЬ ПО РОЛЯМ**: Определи кто говорил (администратор или пациент) по контексту
   - Администратор: приветствует ("Алло", "Здравствуйте", "Клиника Мирамед"), задаёт вопросы о боли, предлагает запись, называет цену
   - Пациент: отвечает на вопросы, описывает симптомы, спрашивает про цену, рассказывает историю
2. **ВОССТАНОВИТЬ ПОРЯДОК**: Разбей на реплики (кто что сказал по очереди)
3. **ПЕРЕВЕСТИ НА РУССКИЙ**: Каждую реплику переведи на чистый русский (если была на казахском)
...
# КОНТЕКСТ КЛИНИКИ МИРАМЕД
- Лечение суставов и позвоночника без операции
- Оффер: "Экспертная диагностика" = 9 900 тенге

# ПРАВИЛА
1. Переводи ДОСЛОВНО — не выдумывай, не добавляй от себя
2. Каждая реплика = одна фраза или группа фраз от одного говорящего
3. Убирай повторы и мусор распознавания
4. НЕ объединяй вопрос и ответ в одну реплику
...
# ФОРМАТ ОТВЕТА — строго JSON, без markdown:
{
  "dialogue": [
    { "role": "manager", "text": "Алло, здравствуйте!" },
    { "role": "client", "text": "Здравствуйте." },
    { "role": "manager", "text": "Клиника Мирамед, меня зовут Айгуль." }
  ]
}

ВАЖНО: role может быть только "manager" или "client".`;

  logger.info('🧠 GPT-4o: реконструкция диалога из моно + перевод...');
  ...
  try {
    ...
    if (match) {
      ...
      logger.warn('⚠️ GPT не вернул JSON, fallback к одному блоку', {
        contentPreview: content.substring(0, 200)
      });
      return { dialogue: [{ role: 'manager', text: rawText }] };
    }
  ...
  logger.info(`✅ Моно-диалог восстановлен`, {
    replicas: dialogue.length,
    firstReplica: dialogue[0]?.text?.substring(0, 50)
  });
...
// ====================================================================
//  ГЛАВНАЯ ФУНКЦИЯ ТРАНСКРИБАЦИИ v5.1
//
//  Pipeline: gpt-4o-transcribe (каждый канал) → GPT-4o (перевод + реконструкция)
//  Вывод: formatted = [{ role: 'manager', text: '...' }, { role: 'client', text: '...' }, ...]
//  Массив реплик диалога по очереди (не два блока!)
// ====================================================================
...
async function transcribeAudio(audioUrl) {
  try {
    ...
    if (FFMPEG_AVAILABLE) {
      try {
        ...
        if (channels) {
          ...
          // GPT-4o: перевод + реконструкция диалога репликами
          const translated = await repairAndTranslate(managerRaw, clientRaw);

          const formatted = translated.dialogue || [];

          const plainText = formatted.map(r => r.text).join(' ');
          logger.info(`✅ Стерео pipeline v5.1 done`, { replicas: formatted.length });
    ...
    // GPT-4o: перевод + разделение по ролям + реконструкция диалога репликами
    const translated = await repairAndTranslateMono(rawText);

    const formatted = translated.dialogue || [];

    const finalPlain = formatted.map(r => r.text).join(' ');
    logger.info(`✅ Mono pipeline v5.1 done`, { replicas: formatted.length });
